...
st.set_page_config(
    ...
    page_icon="https://www.media.stellantis.com/uploads/br/brand/citroen_footer-6332adc17cb38_footer.png",
    layout="wide",
...
# ── Constantes ─────────────────────────────────────────────────────────────────
SHEET_ID = "1DahJRzrS5DUCbg4_z2bVagQsIcLFkVqsmazFZX3l52k"
GID      = "0"
...
with st.sidebar:
    ...
    <div style="text-align:center;padding:16px 0 8px 0">
      <img src="https://www.media.stellantis.com/uploads/br/brand/citroen_footer-6332adc17cb38_footer.png" width="100"><br>
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Registros con empresa=1 (para gráficos)
    df_corp = df[pd.to_numeric(df["empresa"], errors="coerce") == 1].copy()
    
    # Totales sobre TODOS los datos (sin filtros) para % sobre total
    df_total_raw = df_raw.copy()
    registros_corp_total = int((pd.to_numeric(df_total_raw["empresa"], errors="coerce") == 1).sum())
    total_registros = len(df_total_raw)
    
    # Clientes únicos de empresa (desde df_corp filtrado)
    total_corp = df_corp["cl_k_cliente"].nunique() if "cl_k_cliente" in df_corp.columns and len(df_corp) > 0 else 0
    
    # Compras de empresa (registros en df_corp con los filtros aplicados)
    compras_corp = len(df_corp)
    
    # % sobre total (siempre sobre los datos sin filtros)
    pct_corp = round(registros_corp_total / total_registros * 100, 1) if total_registros > 0 else 0
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-card">
        ...
        <div class="kpi-sub">Registros empresa / Total (sin filtros)</div>
    ...
    with col1:
        ...
        if len(df_corp) > 0:
            ...
            st.info("No hay registros con empresa=1 en los filtros actuales.")

    with col2:
        if "cl_dir_provincia" in df_corp.columns and len(df_corp) > 0:
            ...
            st.info("No hay datos de provincia o registros con empresa=1.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    df_gen = df_raw.copy()
    if modelo_sel:
        df_gen = df_gen[df_gen["am_modelocl"].isin(modelo_sel)]
    if provincia_sel and "cl_dir_provincia" in df_gen.columns:
        df_gen = df_gen[df_gen["cl_dir_provincia"].isin(provincia_sel)]
